...
Создаёт обычное текстовое сообщение очевидца или сотрудника. Если отправитель — сотрудник, после сохранения backend отправляет FCM-уведомление на актуальный токен устройства очевидца этого чата. Шаблонные ответы остаются доступными отдельным маршрутом.
...
- `file` — JPEG/PNG до 10 МБ либо GIF/MP4/MOV до 100 МБ.
...
Заголовок `Authorization: Bearer <access_token>` обязателен и при загрузке изображения через Coil/Glide. Относительный `download_url` из сообщения нужно соединить с `BASE_URL`; напрямую открывать его без заголовка нельзя.
...
Управлять банами может любой сотрудник с активной ролью: `inspector`, `administrator` или `chief`.
...
## PUT /api/v1/devices/{target_device_id}/role — выдача роли по значению из QR

Приложение сотрудника формирует QR-код, содержащий **только строку `device_id`**. ZXing или ML Kit используются на Android. Сканер возвращает обычную строку UUID, и frontend передаёт на backend именно это значение как `target_device_id`.

Backend **не получает и не хранит изображение QR**, PNG, Base64 или содержимое камеры. На сервер передаётся только UUID, например `c1e16b9e-d317-444a-ba3e-77fb2da730a2`. Администратор или Начальник отправляет его в path-параметре:
...
1. Устройство без роли показывает QR со своим `device_id`.
2. Управляющее устройство сканирует QR и проверяет, что результат является UUID.
3. UUID подставляется вместо `{target_device_id}`.
4. `requester_device_id` в JSON — устройство Администратора или Начальника, выполняющего действие.
5. Bearer-токен также принадлежит управляющему устройству.
...
- очевидцу — при обычном или шаблонном ответе сотрудника;
...
1. Использовать только `https://силенок.рф:4402/` и `wss://силенок.рф:4402/`.
2. На старте вызвать регистрацию и сохранить `device_id`, связанные ID, `role` и `access_token`.
3. Добавлять `Authorization: Bearer <access_token>` во все HTTP-запросы, кроме регистрации и health check.
4. Не подставлять чужой `device_id`: backend сравнивает его с токеном и возвращает `403`.
5. При `401` повторно зарегистрировать то же устройство с тем же fingerprint и сохранить новый токен.
6. Для WebSocket передавать токен в `?token=`, а не `device_id`.
7. Для медиа добавлять Bearer-заголовок также в Coil/Glide.
...
- `400` — некорректный файл или бизнес-параметр;
- `401` — токен отсутствует, неверен или истёк;
- `403` — нет роли/доступа либо `device_id` не совпадает с токеном;
- `404` — сущность или зарегистрированное устройство не найдены;
- `409` — конфликт текущего состояния;
- `422` — JSON, UUID или обязательные поля не прошли валидацию.
