...
После развёртывания API доступен на публичном порту `4402`:

```text
http://SERVER_IP:4402
http://SERVER_IP:4402/docs
```

Nginx принимает внешние запросы на порту `4402` и передаёт их FastAPI на внутренний адрес `127.0.0.1:8001`.
